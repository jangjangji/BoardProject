...
public class JoinValidator implements Validator<RequestJoin>, RequiredValidator, EmailValidator {
    ...
    public void check(RequestJoin form) {
        ...
        /*필수 항목 유효성 검사 E*/
        //이메일 형식 체크
        checkTrue(checkEmail(email), new AlertException("이메일 형식이아님",status));
        //이메일 중복 여부 체크 - 이미가입된 회원인지 여부
        checkTrue(mapper.exists(email) == 0L,new AlertException("이미 다입된 이메일 입니다.",status));
        //비밀번호 자리수는 8자리 이상
        checkTrue(password.length() >=8, new AlertException("비밀번호는 8자리 이상입력하세요",status));
        //비밀번호와 비밀번호 확인의 일치 여부
        checkTrue(password.equals(confirmPassword),new AlertException("비밀번호가 일치하지않음",status));
    }
